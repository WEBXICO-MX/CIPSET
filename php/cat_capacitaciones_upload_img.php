...
if (isset($_POST['xAccion2'])) {
    if ($_POST["xAccion2"] == "upload") {
        ...
        $cve_capacitacion = isset($_POST['xCveCapacitacion']) ? $_POST['xCveCapacitacion'] : 0;

        $target_dir = "../img/capacitaciones/";
        $target_file = $target_dir . basename($_FILES["fileToUpload"]["name"]);
        $fileType = pathinfo($target_file, PATHINFO_EXTENSION);
        $size = $_FILES["fileToUpload"]["size"];

        /* RENOMBRADO DEL ARCHIVO CON LA CVE */
        $file_name_new = ("$cve_capacitacion.$fileType");
        ...
        if ($fileType != "jpg" && $fileType != "jpeg" && $fileType != "png" && $fileType != "gif") {
            ...
            if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"],$target_file_new)) {
                ...
                $sql = "UPDATE capacitaciones SET img = '" . $valor . "' WHERE id = $cve_capacitacion";
...
<div class="modal-header">
    ...
    <h4 class="modal-title">Capacitación | Subir imagen</h4>
</div>
    ...
    <div class="te">
        <form role="form" action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post" enctype="multipart/form-data" id="frmUpload" name="frmUpload">
            <div class="form-group">
                <input type="hidden" id="xCveCapacitacion" name="xCveCapacitacion" value="<?php echo($_GET["xCveCapacitacion"]); ?>" />

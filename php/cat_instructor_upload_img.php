...
if (isset($_POST['xAccion2'])) {
    if ($_POST["xAccion2"] == "upload") {
        ...
        $cve_instructor = isset($_POST['id']) ? $_POST['id'] : 0;

        $target_dir = "../img/instructores/";
        $target_file = $target_dir . basename($_FILES["fileToUpload"]["name"]);
        $fileType = pathinfo($target_file, PATHINFO_EXTENSION);
        $size = $_FILES["fileToUpload"]["size"];

        /* RENOMBRADO DEL ARCHIVO CON LA CVE */
        $file_name_new = ("$cve_instructor.$fileType");
        ...
        if ($fileType != "jpg" && $fileType != "jpeg" && $fileType != "png" && $fileType != "gif") {
            ...
            if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file_new)) {
                ...
                $sql = "UPDATE instructores SET ruta_foto = '" . $valor . "' WHERE id = $cve_instructor";
        ...
        if (!$exito) {
            ...
            header('Location: cat_instuctor.php');
            return;
        }
    }
}
...
<div class="modal-header">
    ...
    <h4 class="modal-title">Instructor(es) | Subir imagen</h4>
</div>
